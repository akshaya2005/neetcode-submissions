class Solution:
    def kClosest(self, points: List[List[int]], k: int) -> List[List[int]]:
        ...
        def partition(l, r, points):
            ...
            while i <= j:
                while i <= j and distance(points[i]) <= distance(points[l]):
                    i += 1
                while i <= j and distance(points[j]) >= distance(points[l]):
                    j -= 1
                if i <= j:
                    ## swap i and j
                    temp = points[i]
            ...
            if j + 1< k:
                return partition(j + 1, r, points)
            else:
                return partition(l, j - 1, points)
        ...
        return partition(0, len(points) - 1, points)
